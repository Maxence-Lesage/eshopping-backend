      ...
      (err) => {
        if (err) {
          if (err.errno === 1062) {
            res
              .status(409)
              .json({ error: "The email already exist on the server" });
            return;
          }
...
router.post("/login", async function (req, res, next) {
  ...
  try {
    db.query("SELECT * FROM users WHERE email = ?", [email], (err, resDB) => {
      ...
      if (!resDB[0].username) {
        res.status(401).json({ error: "Account do not exist" });
        return;
      }
      ...
      const token = jwt.sign({ userId: resDB[0].id }, "Ad8r5RE41Gf4ha942R4Z", {
        expiresIn: "1h",
      });
      res
        .status(201)
